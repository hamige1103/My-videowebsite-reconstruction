<template>
  <div class="search-type" >
    <!-- 筛选条件显示区域 -->
    <div v-if="hasActiveFilters" class="active-filters" style="margin: 10px 0; padding: 10px; background: #f5f7fa; border-radius: 4px;">
      <span style="font-weight: bold; margin-right: 10px;">当前筛选条件:</span>
      <el-tag 
      ...
      <el-button size="small" @click="clearAllFilters" style="margin-left: 10px;">清除所有</el-button>
    </div>
...
export default {
  // 无限滚动展示视频列表
    ...
    computed: {
      // 计算是否有活跃的筛选条件
      hasActiveFilters() {
        return this.vod_class !== '' || this.vod_area !== '' || this.vod_year !== '';
      },
      ...
      activeFilters() {
        const filters = [];
        if (this.vod_class !== '') {
          filters.push({ key: 'vod_class', name: '类型', value: this.vod_class });
        }
        if (this.vod_area !== '') {
          filters.push({ key: 'vod_area', name: '地区', value: this.vod_area });
        }
        if (this.vod_year !== '') {
          filters.push({ key: "vod_year", name: "年份", value: this.vod_year });
        }
        return filters;
      }
    },
    ...
    methods: {
        ...
        getMovList() {
          ...
          console.log('MovieCardList API请求参数:', param)
          apiGetMovList(param).then(
            (res) => { 
              console.log('MovieCardList API响应:', res)
              console.log('MovieCardList API响应数据:', res.data)
              if (res.data && res.data.code === 200 && res.data.data && res.data.data.length > 0) {
                console.log('MovieCardList 获取到数据，数量:', res.data.data.length)
                ...
                console.log('MovieCardList 没有数据或API错误:', res.data)
            ...
            (error) => {
                console.error('MovieCardList API调用错误:', error)
                this.contentshow = false
                this.infiniteMsgShow = false
                this.disabled = true
            }
